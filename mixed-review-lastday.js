...
function lunch() {
  ...
  // Your code here
  let randomCafe =  cafes[randomInt(0 , cafes.length-1)];
...
function football() {
	let score =  0;
	let user = readline.question("Enter either touchdown or field goal : ");
	if(user.toLowerCase() == "touchdown" || user == "td"){
		score+=7;
		//console.log(score);
	} else if( user.toLowerCase() == "field goal" || user == "fg" || user == "fieldgoal"){
		score+=3;
		//console.log(score);
	}
	return score;
...
function duckDuckGoose() {
	let duck  = "duck" ;
	let goose = "goose";
	let games = [duck , goose];
	let game =  games[randomInt(0 , games.length-1)];
	if(game ==  duck){
		console.log(game);
	} else{
		console.log(game*10);
