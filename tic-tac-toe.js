...
function checkColumns() {
  ...
  if (firsColumnResult != null){
          return board[0][0];
      } else if (secondColumnResult != null){
          return board[0][1];
      } else if (ThirdColumnResult != null){
          return board[0][2];
      } else {
          return null;
      }
...
function checkDiagonals() {
  let firstDiagonalsResult = isTicTacToe(board[0][0], board[1][1], board[2][2]);
  let secondDiagonalsResult = isTicTacToe(board[0][2], board[1][1], board[2][0]);
  if (firstDiagonalsResult != null){
          return board[0][0];
      } else if (secondDiagonalsResult != null){
          return board[0][2];
      } else {
          return null;
      }
...
console.log("welcome Players, lets play some tic-tac-toe!");
...
function playerMove(){
   let pickRow = parseInt(readline.question("Please enter the number of the row you would like , 0-2 zero being the first row and 2 the last : "));
   let pickColumn = parseInt(readline.question("Please enter the number of the column you would like , 0-2 zero being the first column and 2 the last : "));
   while(isValid(pickRow , pickColumn))
   board[pickRow][pickColumn] = player ;
